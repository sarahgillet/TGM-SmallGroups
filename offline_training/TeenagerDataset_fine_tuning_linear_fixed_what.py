...
import wandb
import torch
from dataset import SummerschoolDatasetFixedSize
import MMPN
...
FLAGS_DEF = utils.define_flags_with_default(
    ...
    project_config_name='Teenager-SelectedFeatures-ExploreFineTuning_fixed',
...
def get_data_and_train(dataset_org, kept_indices_manual_selection, feature_sel_dict, training_dataset, FLAGS, finetune=False, path=''):
    ...
    model, result, val_result, test_result, run_id, num_params, path  = utils.train_graph_classifier(model_name="FixedLengthNetwork-Type", wandb_project_name=FLAGS.wandb_project_name, 
...
def main(argv):
    ...
    dataset_org=SummerschoolDatasetFixedSize("../training_data", "allepisodes_norm_no_permutations_norm.csv", 

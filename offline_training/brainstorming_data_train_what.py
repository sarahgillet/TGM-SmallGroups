...
FLAGS_DEF = utils.define_flags_with_default(
    ...
    project_config_name='BrainstormingExplore_Original_what',
    google_sheet_id=1, # give ID of sheet starting with 1
    wandb_project_name="Brainstorming_Experimental_what_v0_2_3",
...
def main(argv):
    ...
    dataset_org=BrainstormingDataset("../training_data/", "annotated_data_what_all/",type='what_all')
    utils.set_random_seed(FLAGS.h_SEED)
    if FLAGS.limit_train:
        train_dataset, test_dataset, valid_dataset, split_type = utils.generateTrainTestValidDataset(
            ...
            dataset_size_train_val=FLAGS.limit_amount_of_train_val, dataset_size_test=FLAGS.limit_amount_test)
    else:
    ...
    print(train_dataset.data.y_what)
    scaler_n = preprocessing.StandardScaler().fit(train_dataset.data.x)
    print(scaler_n.mean_, scaler_n.var_)
    ...
    model, result, val_result, test_result, run_id, num_params, path = utils.train_graph_classifier(model_name="Brainstorming-MMPN-What", wandb_project_name=FLAGS.wandb_project_name, 
    ...
    utils.writeToGoogleSheetsSimple(FLAGS.project_config_name, FLAGS.google_sheet_id-1, run_id, 
                        ...
                        FLAGS.group_embedding_dim, FLAGS.pooling_operation, FLAGS.dropout, use_drive=FLAGS.use_drive)                                      
    wandb.finish()
    if not FLAGS.do_test:
        return
    dataset_org=BrainstormingDataset("../training_data/raw", "data_aggregated_four_what/", test_data=True, group_sizes_train="4", type='what')
    utils.set_random_seed(FLAGS.h_SEED)
    dataset_org.transform_scaler = [scaler_n, scaler_e, scaler_g]
    ...
    model, result, val_result, test_result, run_id_test, num_params = utils.train_graph_classifier(model_name="Brainstorming-MMPN-What", wandb_project_name=FLAGS.wandb_project_name, 
                                        h_SEED=FLAGS.h_SEED, device=FLAGS.device,
    ...
    utils.writeToGoogleSheetsSimple(FLAGS.project_config_name, FLAGS.google_sheet_id, run_id, 
                        FLAGS.h_mess_arch_1, FLAGS.h_node_arch_1, FLAGS.h_mess_arch_2, FLAGS.h_node_arch_2, 

...
def main(argv):
    ...
    for fold in range(FLAGS.num_folds):
        ...
        model, result, val_result, test_result, run_id, num_params, path = utils.train_graph_classifier(model_name="Brainstorming-Linear-Who", wandb_project_name=FLAGS.wandb_project_name, 
    ...
    for i in range(len(data_over_folds_test['path'])):
        ...
        dataset_org=BrainstormingDatasetFixedSize("../training_data/raw", "data_aggregated_four/", test_data=True, group_sizes_train="4", aggr_func=FLAGS.pooling_operation)
        utils.set_random_seed(FLAGS.h_SEED)
        ...
        model, result, val_result, test_result, run_id, num_params = utils.train_graph_classifier(model_name="Brainstorming-Linear-Who", wandb_project_name=FLAGS.wandb_project_name, 
                                            h_SEED=FLAGS.h_SEED, device=FLAGS.device,
                                            graph_train_loader=graph_train_loader, graph_val_loader=graph_val_loader,
                                            graph_test_loader=graph_test_loader,
    ...
    utils.writeToGoogleSheetsCrossValSummaryLinear(FLAGS.project_config_name, FLAGS.google_sheet_id+1, data_over_folds_test,
                        FLAGS.arch,
                        split_type, FLAGS.h_SEED, FLAGS.h_BATCH_SIZE, FLAGS.loss_module,
                        FLAGS.limit_train if not FLAGS.limit_train else 'Groups of '+str(FLAGS.group_size_training)+' , #Ep: '+str(FLAGS.limit_amount_of_train_val),
                        FLAGS.pooling_operation, FLAGS.dropout, use_drive=FLAGS.use_drive)
